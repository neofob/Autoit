...
Func Automate_WebBrowsing()
   Local $openscope = 'http://www.openscope.co/'
   Local $ie_atc = OpenInternetExplorer($openscope)
   Sleep(1000)
   Local $ie_general = OpenInternetExplorer('www.google.com')
   Sleep(1000)
   Local $chrome_atc = Chrome_Start()
   Chrome_Navigate($chrome_atc, $openscope)
   Sleep(1000)
   ...
   For $i = 0 To 5 Step 1
      ...
      Switch Random(0,3,1)
         ...
         Case 2
            Chrome_Navigate($chrome_atc, $openscope)
         Case 3
            _IENavigate($ie_atc, $openscope)
      EndSwitch
   Next
   CloseInternetExplorer($ie_atc)
   CloseInternetExplorer($ie_general)
   Chrome_End($chrome_atc)
